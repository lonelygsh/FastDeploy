...
class TestDraftModelSetValueByFlags(unittest.TestCase):
    ...
    def test_basic_update(self):
        ...
        New semantics:
          - Decoder (step_idx > 0): pre_ids[step_idx + i] = draft_token[i]
          - Encoder (step_idx == 0): pre_ids[0] = draft_token[0]
        """
        bs = 2
        pre_id_length = 6
        ...
        # batch0: decoder step (step_idx=2, 2 history tokens), batch1: encoder step (step_idx=0)
        step_idx = paddle.to_tensor([2, 0], dtype="int64")
        ...
        # batch0: writes draft_token[0,1,2] to pre_ids[2,3,4]
        # batch1: writes draft_token[0] to pre_ids[0]
        expected = np.array([[0, 0, 10, 11, 12, 0], [20, 0, 0, 0, 0, 0]], dtype=np.int64)
    ...
    def test_stop_flags(self):
        ...
        seq_lens_encoder = paddle.to_tensor([0, 0], dtype="int32")
        seq_lens_decoder = paddle.to_tensor([0, 0], dtype="int32")
        # batch0: stopped, batch1: decoder step (step_idx=2)
        ...
        # batch1: writes draft_token[0,1] to pre_ids[2,3]
        expected = np.array([[0, 0, 0, 0, 0], [0, 0, 7, 8, 0]], dtype=np.int64)
